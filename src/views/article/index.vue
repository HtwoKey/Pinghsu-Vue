  ...
  <div>
    <article class="main-content page-page" itemscope itemtype="http://schema.org/Article">
      ...
      <div id="post-content" class="post-content" itemprop="articleBody">
        <!-- 循环文章标签 -->
        <p class="post-tags">
          <router-link v-for="(tag, index) in tagList" :key="index" tag="a" :to="{ 'path':'/tag/'+tag.id }">
            {{ tag.name }}
          </router-link>
        </p>
        <!-- 文章的正文 -->
        <div ref="content" v-html="entity.contentHtml" />
        <p class="post-info"> 本文由
          <a href="#">HTwoKey</a> 创作，采用
    ...
    <app-comments v-if="entity.commentSwitch === 'true'" :aid="id" />
    <!-- 评论组件 -->
  </div>
</template>
